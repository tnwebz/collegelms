...
const CourseBuilder = () => {
  ...
  const [videoSource, setVideoSource] = useState<"youtube" | "drive" | "direct">("youtube");
  const [quizSource, setQuizSource] = useState<"link" | "manual">("link");
  ...
  const handleAddModule = async () => {
    ...
    try {
      ...
      triggerToast("Error adding module", "error");
    }
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      triggerToast("Error publishing course.", "error");
    } finally {
  ...
  const updateActiveProblem = (field: keyof CodeProblem, value: any) => {
    ...
    // @ts-ignore
    updatedProblems[activeProblemIndex] = {
      ...updatedProblems[activeProblemIndex],
      [field]: value,
    };
  ...
  const removeProblem = (index: number) => {
    if (problems.length === 1)
      return triggerToast("At least one problem is required.", "error");
  ...
  const addTestCase = () => {
    ...
    updated[activeProblemIndex].testCases.push({ input: "", output: "" });
    setProblems(updated);
  };
  ...
  const updateTestCase = (
    ...
    const updated = [...problems];
    updated[activeProblemIndex].testCases[caseIdx][field] = value;
    setProblems(updated);
  };

  const removeTestCase = (caseIdx: number) => {
    const updated = [...problems];
    if (updated[activeProblemIndex].testCases.length === 1) return;
    updated[activeProblemIndex].testCases.splice(caseIdx, 1);
    setProblems(updated);
  };

  // --- QUIZ MANAGEMENT LOGIC ---
  const updateActiveQuiz = (field: keyof QuizQuestion, value: any) => {
    const updated = [...quizQuestions];
    updated[activeQuizIndex] = { ...updated[activeQuizIndex], [field]: value };
    setQuizQuestions(updated);
  };

  const updateQuizOption = (optIdx: number, value: string) => {
    const updated = [...quizQuestions];
    updated[activeQuizIndex].options[optIdx] = value;
    setQuizQuestions(updated);
  };

  const addQuizQuestion = () => {
    if (quizQuestions.length >= 30) {
      triggerToast("Maximum 30 questions allowed per quiz.", "error");
      return;
    }
    setQuizQuestions([...quizQuestions, { question: "", options: ["", "", "", ""], correctOptionIndex: 0 }]);
    setActiveQuizIndex(quizQuestions.length);
  };

  const removeQuizQuestion = (index: number) => {
    if (quizQuestions.length === 1) return triggerToast("At least one question is required.", "error");
  ...
  const saveContentItem = async () => {
    if (!selectedModuleId)
      return triggerToast("Select a module from the sidebar first!", "error");
    if (!itemTitle.trim())
      return triggerToast("Please enter a title for this item.", "error");
    ...
    if (activeModal === "Live Class") {
      try {
        ...
        await axios.post(`${API_BASE_URL}/content`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    // 🔴 SPECIAL LOGIC FOR DIRECT VIDEO UPLOAD (Local Backend)
    if (activeModal === "Video" && videoSource === "direct" && uploadFile) {
      try {
        const formData = new FormData();
        formData.append("video", uploadFile);
        formData.append("title", itemTitle || "Uploaded Video");
        if (duration) formData.append("duration", duration);
        formData.append("is_mandatory", isMandatory.toString());
        if (itemInstructions) formData.append("instructions", itemInstructions);
        if (selectedModuleId) formData.append("module_id", selectedModuleId.toString());
        if (startTime) formData.append("start_time", startTime);
        if (endTime) formData.append("end_time", endTime);

        // Optional: you can manually compute progress with axios using onUploadProgress
        await axios.post(`${API_BASE_URL}/content/video`, formData, {
          headers: { 
        ...
        triggerToast(`✅ Direct Video uploaded successfully!`, "success");
        setActiveModal(null);
        resetForm();
        return;
      } catch (err) {
    ...
    // 🔴 SPECIAL LOGIC FOR DOCUMENT UPLOAD (Notes & Assignments)
    if ((activeModal === "Note" || activeModal === "Assignment") && uploadFile) {
      try {
        const formData = new FormData();
        formData.append("document", uploadFile);
        formData.append("title", itemTitle || "Uploaded Document");
        formData.append("type", activeModal === "Note" ? "direct_document" : "assignment_document");
        formData.append("is_mandatory", isMandatory.toString());
        if (itemInstructions) formData.append("instructions", itemInstructions);
        if (selectedModuleId) formData.append("module_id", selectedModuleId.toString());
        if (startTime) formData.append("start_time", startTime);
        ...
        await axios.post(`${API_BASE_URL}/content/document`, formData, {
          headers: { 
            Authorization: `Bearer ${token}`,
          },
          onUploadProgress: (progressEvent) => {
        ...
        triggerToast("Failed to upload document to server.", "error");
        setUploadProgress(null);
        return;
      }
    }
    ...
    if (activeModal === "Video") {
      if (videoSource === "drive") finalTypeKey = "drive_video";
      else if (videoSource === "direct") finalTypeKey = "direct_video";
      else finalTypeKey = "video";
    }
    ...
    if (activeModal === "Code Test") {
      for (let i = 0; i < problems.length; i++) {
        if (!problems[i].title.trim())
          return triggerToast(`Problem ${i + 1} is missing a title!`, "error");
        if (!problems[i].description.trim())
          return triggerToast(
            `Problem ${i + 1} is missing a description!`,
            "error",
          );
      }
    ...
    if (activeModal === "Quiz" && quizSource === "manual") {
      for (let i = 0; i < quizQuestions.length; i++) {
        if (!quizQuestions[i].question.trim())
          return triggerToast(`Question ${i + 1} is missing text!`, "error");
        if (quizQuestions[i].options.some(opt => !opt.trim()))
          return triggerToast(`Question ${i + 1} has empty options!`, "error");
      }
      const config = { quizQuestions: quizQuestions };
      payload.test_config = JSON.stringify(config);
    } else if (activeModal === "Quiz" && quizSource === "link") {
      if (!itemUrl.trim()) return triggerToast("Please enter a link.", "error");
    }
  ...
  const CodingCourseBuilder = () => {
    ...
    const handleAutoFill = async () => {
      ...
      setLoadingAI(true);
      try {
        const res = await axios.post(`${API_BASE_URL}/ai/generate-challenge`, {
          title: cTitle,
        });
        setCDesc(res.data.description);
        const parsedTests =
          typeof res.data.test_cases === "string"
            ? JSON.parse(res.data.test_cases)
            : res.data.test_cases;
        setCTests(parsedTests);
    ...
    const saveChallenge = async () => {
      if (!cTitle || !cDesc)
        return triggerToast("Title and Description required", "error");
      ...
      try {
        ...
        if (editingId) {
          ...
          await axios.patch(
            `${API_BASE_URL}/challenges/${editingId}`,
            payload,
            { headers: { Authorization: `Bearer ${token}` } },
          );
        ...
        triggerToast(
          err.response?.data?.detail || "Error saving problem",
          "error",
        );
      }
    };
        ...
        <header className="flex justify-between items-center bg-white px-4 py-4 md:px-10 border-b border-slate-200 z-50 sticky top-0">
          <div className="flex items-center gap-3 md:gap-5">
            <button
              ...
              className="bg-slate-200 border-none p-2.5 rounded-full cursor-pointer hover:bg-slate-300 transition-colors"
            >
          ...
          <div className="flex items-center gap-2 md:gap-3 text-xs md:text-sm">
            ...
            <button
              ...
              className="px-3 py-2 md:px-5 md:py-2.5 bg-white text-[#005EB8] border border-[#005EB8] rounded-lg font-bold hover:bg-blue-50 transition-colors whitespace-nowrap"
            >
              Preview
            </button>
            <button
              ...
              className="px-3 py-2 md:px-6 md:py-2.5 rounded-lg border-none bg-[#94A3B8] text-white font-extrabold shadow-md hover:bg-[#76a928] transition-all disabled:opacity-50 whitespace-nowrap"
            >
            ...
            {["Easy", "Medium", "Hard"].map((tab) => (
              <button
                ...
                style={{
                  padding: "12px 40px",
                  borderRadius: "30px",
                  background: activeTab === tab ? brand.green : "white",
            ...
            <div className="bg-white p-5 md:p-8 rounded-2xl border border-slate-300 shadow-sm">
              ...
              >
                <input
                  ...
                  placeholder="Problem Title (e.g. Fibonacci)"
                  ...
                  >
                    ...
                    {cTests.length > 1 && (
                      <X
                        ...
                        cursor="pointer"
                        onClick={() => {
                          const n = cTests.filter((_, idx) => idx !== i);
                          setCTests(n);
                        }}
                      />
                    )}
            ...
            <div style={modalContent}>
              ...
              >
                ...
                <X
                  ...
                  style={{ cursor: "pointer", color: brand.textLight }}
                />
              </div>
              ...
              <button onClick={handleSaveCourseDetails} style={saveButton}>
                Save Changes
              </button>
            </div>
          </div>
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: brand.textLight, fontWeight: "600" }}>
            Loading Builder...
          </p>
        </div>
      </div>
    ...
    <div className="max-w-[1400px] mx-auto bg-slate-200 min-h-screen flex flex-col overflow-x-hidden">
      ...
      <header className="flex justify-between rounded-2xl items-center bg-white px-4 py-4 md:px-10 border-b border-slate-200 z-50 sticky top-0">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <button
            onClick={() =>
              navigate(`/dashboard/course/${courseId}/CoursePreview`)
            }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-10 p-5 lg:p-10 flex-1 overflow-hidden">
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => setActiveModal("Heading")}
              style={{
                color: brand.blue,
                background: "none",
                border: "none",
            ...
            {modules.map((m) => (
              <div
                ...
                style={{
                  ...
                  gap: "12px",
                  padding: "14px",
                  background: selectedModuleId === m.id ? "#E2E8F0" : "white",
                  borderRadius: "12px",
                  border:
                    selectedModuleId === m.id
                      ? `1.5px solid ${brand.blue}`
              ...
              >
                <Layout
                  size={18}
                  color={
                    selectedModuleId === m.id ? brand.blue : brand.textLight
                  }
                />
                <span
                  style={{
        ...
        >
          ...
          <p style={{ color: brand.textLight, marginBottom: "48px" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[850px] mx-auto">
            {[
              {
                type: "Note",
                icon: <Edit3 size={28} color={brand.blue} />,
                desc: "Drive PDF Links",
              },
              {
                type: "Video",
                icon: <Video size={28} color={brand.blue} />,
                desc: "YouTube, Drive, MP4",
              },
              {
                type: "Quiz",
                icon: <HelpCircle size={28} color={brand.blue} />,
                desc: "Google Form Links",
              },
              {
                type: "Code Test",
                icon: <Code size={28} color="#7c3aed" />,
                desc: "Compiler Challenges",
              },
              {
                type: "Assignment",
                icon: <FileText size={28} color={brand.blue} />,
                desc: "PDF projects (Drive)",
              },
              {
                type: "Live Class",
                icon: <Radio size={28} color="#ef4444" />,
                desc: "YouTube Live Link",
              },
              {
                type: "Live Test",
        ...
        <div style={modalOverlay}>
          <div style={modalContent}>
            ...
            {activeModal === "Code Test" ? (
              ...
              >
                <div>
                  <label style={labelStyle}>Overall Test Title</label>
                  <input
                    value={itemTitle}
                  ...
                  {problems.map((_, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: "8px",
                        background:
                          activeProblemIndex === idx ? brand.blue : "#f1f5f9",
                        color:
                ...
                >
                  <div style={{ marginBottom: "15px" }}>
                    ...
                    <input
                      value={problems[activeProblemIndex].title}
                      onChange={(e) =>
                        updateActiveProblem("title", e.target.value)
                      }
                  ...
                  <div style={{ marginBottom: "15px" }}>
                    ...
                    <select
                      value={problems[activeProblemIndex].difficulty}
                      onChange={(e) =>
                        updateActiveProblem("difficulty", e.target.value)
                      }
                      style={inputStyle}
                    >
                      <option>Easy</option>
                  ...
                  <div style={{ marginBottom: "15px" }}>
                    ...
                    <textarea
                      rows={4}
                      value={problems[activeProblemIndex].description}
                      onChange={(e) =>
                        updateActiveProblem("description", e.target.value)
                      }
                      placeholder="Explain the logic required..."
                    ...
                    {problems[activeProblemIndex].testCases.map((tc, idx) => (
                      ...
                      >
                        ...
                        >
                          ...
                          {problems[activeProblemIndex].testCases.length >
                            1 && (
                            <Trash2
                              size={14}
                              color="#ef4444"
              ...
              >
                <div>
                  <label style={labelStyle}>
                    {activeModal === "Heading" ? "Heading Name" : "Item Title"}
                  </label>
                  <input
                    value={itemTitle}
                    onChange={(e) => setItemTitle(e.target.value)}
                ...
                {activeModal === "Quiz" && (
                  ...
                  >
                    <div>
                      <label style={labelStyle}>Overall Quiz Title</label>
                      <input
                        value={itemTitle}
                        onChange={(e) => setItemTitle(e.target.value)}
                    ...
                    <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
                      <button onClick={() => setQuizSource("link")} style={{ flex: 1, padding: "8px", borderRadius: "8px", border: `1px solid ${brand.border}`, background: quizSource === "link" ? brand.blue : "white", color: quizSource === "link" ? "white" : brand.textMain, fontWeight: "600", cursor: "pointer" }}>Google Form Link</button>
                      <button onClick={() => setQuizSource("manual")} style={{ flex: 1, padding: "8px", borderRadius: "8px", border: `1px solid ${brand.border}`, background: quizSource === "manual" ? brand.blue : "white", color: quizSource === "manual" ? "white" : brand.textMain, fontWeight: "600", cursor: "pointer" }}>Manual Quiz Builder</button>
                    </div>

                    {quizSource === "link" ? (
                      <div>
                        <label style={labelStyle}>Google Form / App Script Link</label>
                        <div style={{ position: "relative" }}>
                          <Link
                            size={18}
                            style={{
                      ...
                      <>
                          ...
                          {quizQuestions.map((_, idx) => (
                            <div
                              ...
                              style={{
                                ...
                                borderRadius: "8px",
                                background:
                                  activeQuizIndex === idx ? brand.blue : "#f1f5f9",
                                color:
                                  activeQuizIndex === idx
                                    ? "white"
                                    : brand.textLight,
                        ...
                        >
                          <div style={{ marginBottom: "15px" }}>
                            ...
                            <textarea
                              rows={3}
                              value={quizQuestions[activeQuizIndex].question}
                              onChange={(e) =>
                                updateActiveQuiz("question", e.target.value)
                              }
                          ...
                          <div style={{ marginBottom: "15px" }}>
                            ...
                            {quizQuestions[activeQuizIndex].options.map((opt, idx) => (
                              <div key={idx} style={{ display: "flex", gap: "10px", marginBottom: "8px", alignItems: "center" }}>
                                <input
                                  type="radio"
                                  name={`correctOption_${activeQuizIndex}`}
                                  checked={quizQuestions[activeQuizIndex].correctOptionIndex === idx}
                                  onChange={() => updateActiveQuiz("correctOptionIndex", idx)}
                          ...
                          <p style={{ fontSize: "12px", color: brand.textLight }}>* Select the radio button next to the correct answer.</p>
                        </div>
                      </>
                    )}
                  </div>
                ...
                {activeModal !== "Heading" && activeModal !== "Quiz" && (
                  <>
                    {activeModal === "Video" && (
                      <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
                        <button onClick={() => setVideoSource("youtube")} style={{ flex: 1, padding: "8px", borderRadius: "8px", border: `1px solid ${brand.border}`, background: videoSource === "youtube" ? brand.blue : "white", color: videoSource === "youtube" ? "white" : brand.textMain, fontWeight: "600", cursor: "pointer" }}>YouTube</button>
                        <button onClick={() => setVideoSource("drive")} style={{ flex: 1, padding: "8px", borderRadius: "8px", border: `1px solid ${brand.border}`, background: videoSource === "drive" ? brand.blue : "white", color: videoSource === "drive" ? "white" : brand.textMain, fontWeight: "600", cursor: "pointer" }}>Drive Link</button>
                        <button onClick={() => setVideoSource("direct")} style={{ flex: 1, padding: "8px", borderRadius: "8px", border: `1px solid ${brand.border}`, background: videoSource === "direct" ? brand.blue : "white", color: videoSource === "direct" ? "white" : brand.textMain, fontWeight: "600", cursor: "pointer" }}>Upload MP4</button>
                      </div>
                    )}
                    <div>
                      <label style={labelStyle}>
                        {activeModal === "Assignment"
                          ? "Submission Drive Link OR Upload Prompt PDF"
                          : activeModal === "Note"
                            ? "Google Drive PDF Link OR Upload PDF directly"
                            : activeModal === "Video" && videoSource === "drive"
                              ? "Google Drive MP4 Link"
                              : activeModal === "Video" && videoSource === "direct"
                                ? "Select MP4 File"
                                : "YouTube / Google Form / App Script Link"}
                      </label>
                      {((activeModal === "Video" && videoSource === "direct") || activeModal === "Note" || activeModal === "Assignment") ? (
                        <div style={{ position: "relative", display: "flex", gap: "10px", alignItems: "center" }}>
                          <input
                            type="file"
                            accept={activeModal === "Video" ? "video/mp4" : "application/pdf,image/*"}
                            onChange={(e) => setUploadFile(e.target.files?.[0] || null)}
                            style={{ ...inputStyle, flex: 1 }}
                          />
                          {(activeModal === "Note" || activeModal === "Assignment") && (
                          ...
                          {uploadProgress !== null && (
                            <div style={{ marginTop: "10px", fontWeight: "bold", color: brand.blue }}>
            ...
            <button onClick={saveContentItem} style={saveButton}>
              Save{" "}
              {activeModal === "Code Test"
                ? "Test"
                : activeModal === "Heading"
      ...
      {toast.show && (
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: "13px", color: brand.textLight }}>
              {toast.message}
            </p>
          </div>
          <button
...
const saveButton = {
  ...
  fontWeight: "800",
  cursor: "pointer",
  boxShadow: "0 10px 15px -3px rgba(0, 94, 184, 0.3)",
};
